...
return_type DynamixelHardware::configure(const hardware_interface::HardwareInfo & info)
{
  ...
  mechanical_offsets_.resize(info_.joints.size(), 0.0);
  ...
  for (uint i = 0; i < info_.joints.size(); i++) {
    ...
    if (info_.joints[i].parameters.count("mechanical_offset") > 0) {
      mechanical_offsets_[i] = std::stof(info_.joints[i].parameters.at("mechanical_offset"));
    }
...
return_type DynamixelHardware::read()
{
  ...
  for (uint i = 0; i < ids.size(); i++) {
    joints_[i].state.position =
      dynamixel_workbench_.convertValue2Radian(ids[i], positions[i]) / mechanical_reductions_[i] +
      mechanical_offsets_[i];
    joints_[i].state.velocity =
      dynamixel_workbench_.convertValue2Velocity(ids[i], velocities[i]) / mechanical_reductions_[i];
    joints_[i].state.effort =
      dynamixel_workbench_.convertValue2Current(currents[i]) / mechanical_reductions_[i];
  }
